    ...
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapGet("/module_a", async context =>
                {
                    await context.Response.WriteAsync("You passed the role test - Ayush, Group Name :AD_POC_ModuleA");
                }).RequireAuthorization(new AuthorizeAttribute() { Roles = "AD_POC_ModuleA" });

                endpoints.MapGet("/module_b", async context =>
                {
                    await context.Response.WriteAsync("You passed the role test - Navakanth, Group Name :AD_POC_ModuleB");
                }).RequireAuthorization(new AuthorizeAttribute() { Roles = "AD_POC_ModuleB" });

                endpoints.MapGet("/module_c", async context =>
                {
                    await context.Response.WriteAsync("You passed the role test - Sree Ram Pavan, Group Name :AD_POC_ModuleC");
                }).RequireAuthorization(new AuthorizeAttribute() { Roles = "AD_POC_ModuleC" });
